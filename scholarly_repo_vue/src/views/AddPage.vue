    ...
    <div class='addpage-body d-flex flex-column' style='min-width: 45%; max-width: 45%;'>
      ...
      <section class='black-section hero is-medium mb-6 black-section' id='left-section'>
        <form v-on:submit.prevent='addAuthor'>
          ...
          <div class='image-field d-flex flex-row'>
            <label class='label' style='min-height: 30px; max-height: 30px; padding-top: 5px; margin-right: 5px;'>Image</label>
            <input class='input-image' type='file' accept='image/*' @change='onImageInput'>
          </div>
          ...
          <div class='field'>
            <div class='control'>
              <button class='button' id='add-button'><span class='btn btn-success' style='min-width: 100%; position: absolute; border-radius: 0;'>Submit</span></button>
            </div>
          </div>
        </form>
...
export default {
  ...
  methods: {
    ...
    async addAuthor () {
      ...
      if (this.first_name === '' || this.author_slug === '') {
        this.authorErrors.push("The author's first name and the author slug are required!")
      }
